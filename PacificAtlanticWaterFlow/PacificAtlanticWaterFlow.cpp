...
// Function to check if coordinate (i, j) lies inside N x M matrix
...
// Function to find the count of valid coordinates
	...
	int N, int M)
{
	...
	// Stores the visited coordinates during the 1st traversal
	vector<vector<bool> > visited1(
		N,
		vector<bool>(M, false));

	// Stores the visited coordinates during the 2nd traversal
	vector<vector<bool> > visited2(
		N,
		vector<bool>(M, false));

	// Insert the coordinates directly connected
	for (int i = 0; i < M; i++) {
		q1.push({ 0, i });
		q2.push({ N - 1, i });
	}
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < M; j++) {
			// If coordinate (i, j) is reachable from both
			if (visited1[i][j]
				&& visited2[i][j]) {
				// Update count
				ans++;
